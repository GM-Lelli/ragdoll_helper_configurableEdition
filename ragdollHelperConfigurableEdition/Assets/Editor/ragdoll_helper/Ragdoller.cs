...
namespace BzKovSoft.RagdollHelper.Editor {
	...
	sealed class Ragdoller {
		...
		static void ConfigureJointParams(RagdollPartBase part, Rigidbody anchor, Vector3 axis, Vector3 swingAxis) {
			...
			joint.axis = part.transform.InverseTransformDirection(axis);
			joint.secondaryAxis = part.transform.InverseTransformDirection(swingAxis);
		}

		static void ConfigureJointLimits(ConfigurableJoint joint, float lowTwist, float highTwist, float swing1, float swing2) {
			if (lowTwist > highTwist)
				throw new ArgumentException("wrong limitation: lowTwist > highTwist");

			// Limita il movimento lineare per mantenerlo ancorato
			joint.xMotion = ConfigurableJointMotion.Locked;
			joint.yMotion = ConfigurableJointMotion.Locked;
			joint.zMotion = ConfigurableJointMotion.Locked;

			// Configura il movimento angolare
			joint.angularXMotion = ConfigurableJointMotion.Limited;
			joint.angularYMotion = ConfigurableJointMotion.Limited;
			joint.angularZMotion = ConfigurableJointMotion.Limited;

			// Limiti per il movimento attorno all'asse X (torsione)
			SoftJointLimit lowLimit = new SoftJointLimit();
			lowLimit.limit = lowTwist;
			joint.lowAngularXLimit = lowLimit;

			SoftJointLimit highLimit = new SoftJointLimit();
			highLimit.limit = highTwist;
			joint.highAngularXLimit = highLimit;

			// Limiti per il movimento angolare attorno l'asse di oscillazione Y
			SoftJointLimit swing1Limit = new SoftJointLimit();
			swing1Limit.limit = swing1;
			joint.angularYLimit = swing1Limit;

			// Limiti per il movimento angolare attorno l'asse di oscillazione Z
			SoftJointLimit swing2Limit = new SoftJointLimit();
			swing2Limit.limit = swing2;
			joint.angularZLimit = swing2Limit;

			// Configurazione delle molle dei limiti (equivalente alle molle di twist e swing del `CharacterJoint`)
			SoftJointLimitSpring limitSpring = new SoftJointLimitSpring();
			limitSpring.spring = 0f;  // Modifica questo valore per adattare la rigidità delle molle
			limitSpring.damper = 0f;   // Modifica questo valore per adattare lo smorzamento delle molle
		...
		static void AddComponentesToBase(RagdollPartBase part, RagdollProperties ragdollProperties, float mass, bool addJoint) {
			...
			if (addJoint) {
				...
				part.joint.enablePreprocessing = true;
				part.joint.projectionMode = JointProjectionMode.PositionAndRotation;
				part.joint.projectionDistance = 0.1f; // Modifica questo valore per definire la tolleranza per la correzione della posizione
				part.joint.projectionAngle = 180f; // Modifica questo valore per definire la tolleranza per la correzione della rotazione
    			part.joint.rotationDriveMode = RotationDriveMode.Slerp; // Configura il Rotation Drive Mode a Slerp
			}
		}
